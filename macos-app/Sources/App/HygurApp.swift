...
struct HygurApp: App {
    ...
    var body: some Scene {
        WindowGroup {
            ContentView()
                ...
                .task {
                    ...
                    // Forward unexpected supervisor exits into Activity so the
                    // user notices the failure even if it self-recovers via
                    // backoff. Capturing eventStream by reference is safe —
                    // both objects live for the entire app lifetime.
                    supervisor.onUnexpectedExit = { [eventStream] reason in
                        eventStream.recordLocalIncident(
                            type: "sidecar_restart",
                            message: reason,
                            source: "supervisor"
                        )
                    }
                    // Forward chat / embedding failures (posted via
                    // `.appIncident` from view models that don't hold a
                    // direct EventStreamService reference). The observer
                    // lives for the whole app, so we never deregister.
                    NotificationCenter.default.addObserver(
                        forName: .appIncident,
                        object: nil,
                        queue: .main
                    ) { [eventStream] note in
                        let message = (note.object as? String) ?? "Operation failed"
                        let type = (note.userInfo?["type"] as? String) ?? "app_failed"
                        Task { @MainActor in
                            eventStream.recordLocalIncident(
                                type: type,
                                message: message,
        ...
        .commands {
            ...
            // (which would dead-end into Apple's help viewer with no content).
            // Three useful pointers: shortcuts cheatsheet, repo, and the
            // diagnostics-prefilled issue link reused from Settings.
            CommandGroup(replacing: .help) {
                ...
                Button("Hygur on GitHub") {
                    if let url = URL(string: "https://github.com/hygurlabs/hygur") {
                ...
                Button("Report an Issue…") {
                    Task { @MainActor in
                        let report = await DiagnosticsService.copyToClipboard()
                        if let url = DiagnosticsService.makeIssueURL(report: report.text) {
                            NSWorkspace.shared.open(url)
                        }
                    }
                }
...
extension Notification.Name {
    ...
    /// Reports an app-side incident (chat reply failed, embedding failed) so
    /// EventStreamService can record it in the Activity feed without the
    /// emitter needing a direct reference. `object` is the user-readable
    /// reason; `userInfo["type"]` is the activity type ("chat_failed",
    /// "embedding_failed").
    static let appIncident = Notification.Name("appIncident")
    /// Opens the keyboard shortcuts cheatsheet sheet on the main window.
    /// Posted by Help → Keyboard Shortcuts and ⌘/.
