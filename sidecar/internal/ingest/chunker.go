...
// ChunkOptions configures the chunking behavior (legacy).
// Deprecated: Use ChunkerConfig instead.
...
// DefaultChunkOptions returns the default chunking configuration (legacy).
// Deprecated: Use DefaultChunker() instead.
func DefaultChunkOptions() ChunkOptions {
	return ChunkOptions{
		MaxChunkSize: 500, // 500 chars ~= 166 tokens (conservative for LM Studio's 512 token limit)
		ChunkOverlap: 50,
	}
}

// LegacyChunk represents a piece of a document (legacy).
// Deprecated: Use Chunk instead.
type LegacyChunk struct {
	// Index is the position of this chunk in the document (0-indexed).
	Index int
...
// ChunkText splits text into overlapping chunks (legacy).
// Deprecated: Use Chunker.Chunk() instead.
func ChunkText(text string, opts ChunkOptions) []LegacyChunk {
	if text == "" {
		return nil
	}
	...
	if opts.ChunkOverlap >= opts.MaxChunkSize {
		opts.ChunkOverlap = opts.MaxChunkSize / 2
	}
	...
	for start < textLen {
		...
		nextStart := end - opts.ChunkOverlap
